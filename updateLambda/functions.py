...
def incrementTeam(team_num, attendee_exp):
    """
    ...
    """
    print("Incrementing Team " + str(team_num))
...
def decrementTeam(team_num, attendee_exp):
    """
    Updates the existing team member count
    ...
    attendee_exp: [int] - experience of the registree [0 - none, 1 - low, 2 - low_mid, 3 - medium, 4 - mid_high, 5 - high]
    """
    print("Decrementing Team " + str(team_num))
...
def deleteParticipant(attendee_id, team_num):
    """
    ...
    attendee_exp: [int] - experience of the registree [0 - none, 1 - low, 2 - low_mid, 3 - medium, 4 - mid_high, 5 - high]
    """
    print("Deleting participant from table:" + TABLE_REGISTER)

    #delete participant from ddb
    try:
        response = ddb_client.delete_item(
            TableName = TABLE_REGISTER,
            Key={
                "AttendeeID": {"N": attendee_id},
                "Team":{"N": team_num}
            })
...
def createTeam(team_num, attendee_exp):
    ...
    if key_exists:
        ...
        if len(teams) > 0:
            ...
            createTeamNoRoom(team_num, attendee_exp)
    else:
        createTeamNoRoom(team_num, attendee_exp)
